 ...
 */
export interface RedirectHttpError<TCode extends number = number> extends HttpError<TCode> {
  readonly location: string;
}

/**
 * Checks if the given error is a {@link RedirectHttpError}.
 */
export function isRedirectHttpError(error: any): error is RedirectHttpError {
 ...
 * also uses the {@link isRedirectHttpError} function.
 */
export function generateRedirectHttpErrorClass<TCode extends number>(
  ...
  return class SpecificRedirectHttpError extends BaseClass {
    ...
    public static isInstance(error: any): error is SpecificRedirectHttpError {
      return isRedirectHttpError(error) && error.statusCode === code;
    }
  };
}
