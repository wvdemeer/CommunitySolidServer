...
import { AllowAcceptMetadataWriter } from '../../../../../src/http/output/metadata/AllowAcceptMetadataWriter';
...
describe('An AllowAcceptMetadataWriter', (): void => {
  ...
  const error404 = new RepresentationMetadata({ [RDF.type]: NotFoundHttpError.uri });
  const error405 = new RepresentationMetadata(
    { [RDF.type]: MethodNotAllowedHttpError.uri, [SOLID_ERROR.methodNotAllowed]: 'PUT' },
  );
  const error415 = new RepresentationMetadata({ [RDF.type]: UnsupportedMediaTypeHttpError.uri });
  let response: HttpResponse;
  let writer: AllowAcceptMetadataWriter;

  beforeEach(async(): Promise<void> => {
    response = createResponse();

    writer = new AllowAcceptMetadataWriter(
